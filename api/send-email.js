...
function rateLimit(ip, max = 20, windowMs = 60 * 60 * 1000) {
  const now = Date.now();
...
const templates = {
  'form-code': ({ clientName, formCode, eventType, eventDate, location }) => ({
    ...
    html: `<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;border:1px solid #e8dcc8;border-radius:12px;overflow:hidden;">
        <div style="background:#1A1209;padding:28px 32px;text-align:center;">
          <h1 style="color:#C9A84C;font-size:28px;margin:0;">Quoted.</h1>
        ...
        <div style="padding:32px;background:#FFFDF9;">
          ...
          <p style="color:#7A6652;font-size:15px;line-height:1.6;margin:0 0 24px;">Your photography request has been submitted! Save your form code — you'll need it to leave a review.</p>
        ...
        <div style="background:#F5F0E8;padding:16px 32px;text-align:center;font-size:12px;color:#7A6652;">© ${new Date().getFullYear()} Quoted</div>
      </div>`,
  }),
  ...
  'review-request': ({ clientName, photographerName, studioName, formCode, reviewUrl }) => ({
    ...
    html: `<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;border:1px solid #e8dcc8;border-radius:12px;overflow:hidden;">
        <div style="background:#1A1209;padding:28px 32px;text-align:center;"><h1 style="color:#C9A84C;font-size:28px;margin:0;">Quoted.</h1></div>
        <div style="padding:32px;background:#FFFDF9;">
          <h2 style="color:#1A1209;margin:0 0 12px;">Hi ${sanitize(clientName)}!</h2>
          <p style="color:#7A6652;font-size:15px;line-height:1.6;margin:0 0 24px;"><strong>${sanitize(photographerName)}</strong>${studioName ? ` from <strong>${sanitize(studioName)}</strong>` : ''} is requesting a review. It only takes 2 minutes!</p>
          ...
          <div style="background:#FDF6E3;border-radius:8px;padding:16px;">
  ...
  'password-reset': ({ name, resetUrl }) => ({
    ...
    html: `<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;border:1px solid #e8dcc8;border-radius:12px;overflow:hidden;">
        ...
        <div style="padding:32px;background:#FFFDF9;">
          ...
          <p style="color:#7A6652;font-size:15px;line-height:1.6;margin:0 0 24px;">Hi ${sanitize(name)}, click below to reset your password. This link expires in <strong>1 hour</strong>.</p>
          <div style="text-align:center;margin-bottom:24px;">
          ...
          <div style="background:#FAF7F2;border-radius:8px;padding:14px;margin-bottom:16px;">
            <p style="font-size:12px;color:#7A6652;margin:0 0 4px;">Or paste this in your browser:</p>
            <p style="font-size:12px;color:#C9A84C;word-break:break-all;margin:0;">${sanitize(resetUrl)}</p>
          </div>
          <p style="color:#7A6652;font-size:13px;">If you didn't request this, ignore this email.</p>
        </div>
        <div style="background:#F5F0E8;padding:16px 32px;text-align:center;font-size:12px;color:#7A6652;">© ${new Date().getFullYear()} Quoted · Link expires in 1 hour</div>
      </div>`,
  }),

  'new-lead': ({ photographerName, eventType, location, eventDate, leadCost }) => ({
    subject: `New ${sanitize(eventType)} lead — ${sanitize(location)}`,
    html: `<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;border:1px solid #e8dcc8;border-radius:12px;overflow:hidden;">
        <div style="background:#1A1209;padding:28px 32px;text-align:center;"><h1 style="color:#C9A84C;font-size:28px;margin:0;">Quoted.</h1></div>
        <div style="padding:32px;background:#FFFDF9;">
          ...
          <div style="background:#FDF6E3;border:1.5px solid #C9A84C;border-radius:10px;padding:20px;margin-bottom:24px;">
            <table style="width:100%;border-collapse:collapse;font-size:14px;">
              ...
              <tr><td style="padding:5px 0;color:#7A6652;">Date</td><td style="color:#1A1209;font-weight:600;">${sanitize(eventDate)}</td></tr>
              <tr><td style="padding:5px 0;color:#7A6652;">Cost</td><td style="color:#C9A84C;font-weight:700;font-size:16px;">${sanitize(String(leadCost))} pts</td></tr>
            </table>
          </div>
          <div style="text-align:center;">
            <a href="${process.env.SITE_URL || 'https://quoted-alpha.vercel.app'}" style="display:inline-block;background:#C9A84C;color:#fff;text-decoration:none;padding:14px 32px;border-radius:8px;font-weight:600;">View Lead →</a>
          </div>
        </div>
        <div style="background:#F5F0E8;padding:16px 32px;text-align:center;font-size:12px;color:#7A6652;">© ${new Date().getFullYear()} Quoted</div>
      </div>`,
...
module.exports = async function handler(req, res) {
  ...
  if (!type || !to || !data) return res.status(400).json({ error: 'Missing required fields' });
  if (!templates[type]) return res.status(400).json({ error: `Unknown type: ${type}` });
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(to)) return res.status(400).json({ error: 'Invalid email' });

  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey) {
    console.error('[Quoted] Missing RESEND_API_KEY env var');
  ...
  try {
    ...
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'Quoted <onboarding@resend.dev>',
        to: [sanitize(to, 200)],
        subject,
        html,
      }),
    ...
    if (!response.ok) {
      console.error('[Quoted] Resend error:', JSON.stringify(result));
      return res.status(500).json({ error: 'Failed to send email', detail: result.message });
    }

    console.log(`[Quoted] Email sent: type=${type} to=${to} id=${result.id}`);
    return res.status(200).json({ success: true });

  } catch (err) {
    console.error('[Quoted] Error:', err.message);
